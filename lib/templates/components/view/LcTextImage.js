...
export default {
  ...
  render (h, {props}) {
    ...
    const childs = [h(layoutHeader, {props: {content}})]
...
function wrapLayoutRow (h, element, staticClass) {
  return h('v-layout', {staticClass: staticClass || 'row wrap'}, element)
}

function wrapFlex (h, element, staticClass) {
  return h('v-flex', {staticClass: staticClass || 'xs12 sm6'}, element)
}
